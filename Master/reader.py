...
def readInChunkTransactions(sample):
    unique = {} #items superset
    item_transactions = {} #supports of items
    transactions = {} #storing transactions
    tid = 0
    for c in sample:#counting unique values along with the inclusion counters
        tid += 1
        transactions.update({tid:c.split()})
        for char in c.split():
            if char not in unique:
...
def readInTrunkatedChunkTransactions(sample, numUnique):
    unique = {} #items superset
    item_transactions = {} #supports of items
    transactions = {} #storing transactions
    tid = 0
